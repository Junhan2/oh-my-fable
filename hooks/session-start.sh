...
#   {"enabled": true, "mode": "interactive" | "unattended", "delivery": "hook" | "rules-file" | "claude-md"}
...
# Defaults: enabled, interactive, hook.
#
...
[ "$(val "$GLOBAL" mode)" = "unattended" ] && MODE=unattended
...
printf '%s\n\n(oh-my-fable: mode %s, delivery hook. Change with /fable-setup. Per-request layer: /fable-prompt.)\n' "$BODY" "$MODE"
exit 0
